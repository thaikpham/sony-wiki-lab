...
- local Rust toolchain đã có trên máy hiện tại, nhưng `cargo build/test` vẫn bị chặn bởi thiếu linker host `cc` trên Fedora
- handover test cho Windows đã được chuẩn bị riêng tại `docs/photobooth_windows_handover.md`
- vẫn chưa nên xem toàn bộ stack là production-ready vì Rust host chưa được verify build/test end-to-end trên Windows booth machine
...
| Rust host architecture | ~80% | Session list, event fan-out, asset serving, live-view binary path, file watcher subscription và settings sync đã được nối lại; vẫn thiếu compile verification |
...
- `PATCH /settings` giờ sync lại vào `SessionManager`, không còn lệch config giữa API và capture path
- file detection path đã được gộp để tránh duplicate asset/event registration
- `notify` đã đổi sang cấu hình cross-platform, không còn bị khóa sai vào `macos_kqueue`
- một số lỗi ownership/response typing có xác suất compile fail cao đã được vá trước khi test Windows
...
- chưa verify full flow trên máy Windows có Sony SDK và compiler toolchain đầy đủ

### 4. Toolchain reality

Môi trường Fedora hiện tại đã có:
...
Nên `cargo build` và `cargo test` hiện đều dừng ở lỗi:

```text
error: linker `cc` not found
```

Vì vậy phần Rust host hiện mới được xác nhận ở mức:

- static inspection
- codepath refactor
- ownership/concurrency fixes
- cross-platform dependency cleanup

chứ **chưa có bằng chứng compile/runtime local hoàn chỉnh**.

### 5. Windows handover

Phiên test tiếp theo nên dùng máy Windows booth machine và theo checklist tại:

- `docs/photobooth_windows_handover.md`

Mục tiêu của handover đó là xác nhận lần lượt:

1. `apps/booth-host` build được với `cargo build`
2. `apps/booth-host` test pass với `cargo test`
3. bridge load được
4. host lên được `/status`, `/sessions`, `/ws/events`, `/ws/liveview`
5. full capture flow thực sự chạy qua session -> capture -> file detect -> review/share
